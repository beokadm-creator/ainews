...
4. 기사 개별 작성 원칙 (1:1 맵핑): 제공된 <HTML_SKELETON> 안의 [기사 N] 블록 개수와 구조를 단 하나도 빠짐없이 모두 개별적으로 유지하고 채워야 합니다.
5. 중복·유사 기사 처리: 중복되거나 유사한 주제의 기사라도 절대 임의로 병합(통합)하여 뼈대를 삭제하거나 줄이면 안 됩니다. 동일 딜의 다른 기사일 경우, 해당 기사에서 새롭게 강조하는 보충 사실이나 각도를 중심으로 요약하세요. (뼈대 개수 유지 필수)
