...
export default function MarketInsight() {
  ...
  const [articles, setArticles] = useState<MarketInsightArticle[]>([]);
  ...
  const handleLogin = async () => {
    ...
    setLoginStatus(null);
    try {
      const loginFn = httpsCallable(functions, 'marketinsightLogin');
  ...
  const handleScrape = async () => {
    ...
    setError(null);
    try {
      const scrapeFn = httpsCallable(functions, 'marketinsightScrape');
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">MarketInsight</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">한경 MarketInsight MNA 섹션 스크래핑 (유료 회원)</p>
        </div>
      </div>
