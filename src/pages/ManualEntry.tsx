...
export default function ManualEntry() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">수동 기사 입력</h1>
        <p className="text-gray-600 mt-1">자동 수집이 어려운 기사를 직접 입력하고 AI 분석을 받으세요.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">기사 본문 *</label>
              <textarea 
                value={content}
                onChange={(e) => setContent(e.target.value)}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1e3a5f] outline-none h-64 resize-none"
            ...
            {error && (
              <div className="p-3 bg-red-50 text-red-700 border border-red-200 rounded-lg flex items-start">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col">
          ...
          {isAnalyzing && (
            <div className="flex-1 flex flex-col items-center justify-center text-gray-500 border-2 border-dashed border-gray-200 rounded-lg">
              <Loader2 className="w-8 h-8 animate-spin mb-4 text-[#1e3a5f]" />
              <p>GLM-4 모델이 기사를 분석하고 있습니다...</p>
          ...
          {analysisResult && (
            <div className="space-y-4 flex-1 overflow-y-auto pr-2 custom-scrollbar">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">3줄 요약</label>
                <textarea 
                  value={analysisResult.summary?.join('\n') || ''}
                  onChange={(e) => setAnalysisResult({...analysisResult, summary: e.target.value.split('\n')})}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg outline-none h-24"
                />
              </div>
              
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">거래 규모</label>
                  <input 
                    type="text" 
              ...
              <div className="space-y-2 border border-gray-200 p-3 rounded-lg bg-gray-50">
                <p className="text-sm font-medium text-gray-700">관련 기업 정보</p>
                <div className="grid grid-cols-1 gap-2">
                  <input type="text" placeholder="인수자" value={analysisResult.companies?.acquiror || ''} onChange={(e) => setAnalysisResult({...analysisResult, companies: {...analysisResult.companies, acquiror: e.target.value}})} className="px-3 py-1.5 border rounded text-sm" />
                  <input type="text" placeholder="피인수자" value={analysisResult.companies?.target || ''} onChange={(e) => setAnalysisResult({...analysisResult, companies: {...analysisResult.companies, target: e.target.value}})} className="px-3 py-1.5 border rounded text-sm" />
                  <input type="text" placeholder="재무적 투자자" value={analysisResult.companies?.financialSponsor || ''} onChange={(e) => setAnalysisResult({...analysisResult, companies: {...analysisResult.companies, financialSponsor: e.target.value}})} className="px-3 py-1.5 border rounded text-sm" />
                </div>
              </div>
              
              <div className="pt-4 mt-4 border-t border-gray-100">
                {success && (
                  <div className="mb-3 p-3 bg-green-50 text-green-700 border border-green-200 rounded-lg text-sm text-center">
                    {success}
                  </div>
                )}
                <button 
