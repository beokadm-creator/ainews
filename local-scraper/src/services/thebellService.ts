...
export class ThebellService {
  ...
  async refreshSession(): Promise<boolean> {
    ...
    try {
      const savedCookies = this.loadCookies();
      if (savedCookies.length > 0) await page.setCookie(...savedCookies);
      await page.goto('https://www.thebell.co.kr/', { waitUntil: 'domcontentloaded', timeout: 20000 });
      await new Promise(r => setTimeout(r, 2000 + Math.random() * 2000));
      const cookies = await page.cookies();
      if (cookies.length > 0) {
        this.saveCookies(cookies);
        console.log('[TheBell] Session refreshed');
        return true;
      }
      return false;
    } catch (e: any) {
      console.warn('[TheBell] Session refresh failed:', e.message);
      return false;
    } finally {
      await page.close();
    }
